...
import { ERC20 } from "../../../generated/WorldesDvmProxy/ERC20";
import { ERC20NameBytes } from "../../../generated/WorldesDvmProxy/ERC20NameBytes";
import { ERC20SymbolBytes } from "../../../generated/WorldesDvmProxy/ERC20SymbolBytes";
import {
  DVM,
  DVM__getPMMStateResultStateStruct,
} from "../../../generated/DVMFactory/DVM";
import {
  User,
  Token,
  Pair,
...
export function getWorldes(): Worldes {
  let worldes = Worldes.load(WorldesID);
  if (worldes === null) {
    worldes = new Worldes(WorldesID);
    worldes.pairCount = ZERO_BI;
    worldes.tokenCount = ZERO_BI;
    worldes.crowdpoolingCount = ZERO_BI;
    worldes.txCount = ZERO_BI;
    worldes.volumeUSD = ZERO_BD;
    worldes.feeUSD = ZERO_BD;
    worldes.maintainerFeeUSD = ZERO_BD;
    worldes.DIP3MaintainerFeeUSD = ZERO_BD;
    worldes.uniqueUsersCount = ZERO_BI;
    worldes.updatedAt = ZERO_BI;
    worldes.save();
  }
  return worldes as Worldes;
}
...
export function createUser(address: Address, event: ethereum.Event): User {
  ...
  if (user === null) {
    ...
    worldes.uniqueUsersCount = worldes.uniqueUsersCount.plus(ONE_BI);
    worldes.save();
  }
  return user as User;
}
...
export function createToken(address: Address, event: ethereum.Event): Token {
  ...
  if (token == null) {
    if (address.toHexString() == BASE_COIN) {
      ...
      token.totalLiquidityOnWorldes = ZERO_BD;
    } else {
      token = new Token(address.toHexString());
    ...
    let worldes = getWorldes();
    worldes.tokenCount = worldes.tokenCount.plus(ONE_BI);
    worldes.save();
  }
...
export function updateUserDayDataAndWorldesDayData(
  event: ethereum.Event,
  type: string
): void {
