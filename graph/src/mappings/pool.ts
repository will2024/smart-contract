...
import {
  ...
  getWorldes,
  updateStatistics,
  updateUserDayDataAndWorldesDayData,
...
import { DVM__getPMMStateResultStateStruct } from "../../../generated/DVMFactory/DVM";
import { calculateUsdVolume, updatePrice } from "../utils/pricing";
import { addToken, addTransaction, addVolume } from "../utils/transaction";
import { increaseVolumeAndFee, increaseMaintainerFee } from "../utils/dayUpdates";
...
export function handleWorldesSwap(event: WorldesSwap): void {
  //base data
  ...
  worldes.feeUSD = worldes.feeUSD.plus(feeUSD);
  ...
  updateUserDayDataAndWorldesDayData(event, TRANSACTION_TYPE_SWAP);
...
export function handleBuyShares(event: BuyShares): void {
  ...
  updateUserDayDataAndWorldesDayData(event, TRANSACTION_TYPE_LP_ADD);
}
...
export function handleSellShares(event: SellShares): void {
  ...
  updateUserDayDataAndWorldesDayData(event, TRANSACTION_TYPE_LP_REMOVE);
}
